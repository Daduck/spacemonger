...
- [x] **Step 1: Update `AsyncScanEngine.h` to declare `m_treeMutex` and `GenerateLiveLayout`**

Add `mutable std::mutex m_treeMutex;` and `void GenerateLiveLayout(...)`.

- [x] **Step 2: Implement tree synchronization and `GenerateLiveLayout` in `AsyncScanEngine.cpp`**
...
- [x] **Step 3: Write multi-threaded unit tests in `tests/AsyncScan_test.cpp`**
...
- [x] **Step 4: Build and run test suite**
...
- [x] **Step 1: Declare `UpdateLiveScanLayout` in `FolderView.h`**

Add public method `void UpdateLiveScanLayout(AsyncScanEngine& engine, ui64 totalspace, ui64 freespace);`.

- [x] **Step 2: Implement `UpdateLiveScanLayout` in `FolderView.cpp`**
...
- [x] **Step 3: Add 100ms render tick to `CFolderTree::LoadTree()` in `FolderTree.cpp`**

During the active scan loop, trigger `fv->UpdateLiveScanLayout(engine, totalspace, freespace)` every 100 ms.

- [x] **Step 4: Build SpaceMonger executable and verify compilation**
...
- [x] **Step 1: Run full test suite across all presets**
